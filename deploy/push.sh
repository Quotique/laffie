...
DEPLOY_HOST="10.200.200.6"
DEPLOY_USER="minerva"

cargo build --release --target aarch64-unknown-linux-gnu;

echo "STOPING SERVICE"
ssh $DEPLOY_USER@$DEPLOY_HOST systemctl --user stop minerva.service
...
rsync -zav --rsync-path="mkdir -p /home/$DEPLOY_USER/.config/systemd/user && rsync"\
    deploy/minerva.service $DEPLOY_USER@$DEPLOY_HOST:~/.config/systemd/user/;
...
ssh $DEPLOY_USER@$DEPLOY_HOST systemctl --user start minerva.service;
