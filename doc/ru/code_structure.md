...
| `database` | Хранение задач и истории решений (`sled`). |
| `utils` | Логирование (`slog`), вспомогательные структуры. |
| `cli`, `tui`, `tgbot`, `migrate_db` | Приложения-бинарники. |
...
`src/cli/`, `src/tui/`, `src/migrate_db/`, `src/tgbot/` — приложения,
использующие `solver`, `parser`, `view`, `database`. Конфигурации в
`config/`. Подробности — [`claude_docs/binaries.md`](../../claude_docs/binaries.md).

> `tgbot` и связанные с ним типы `UserDb`/`UserRecord` — устаревший код
> без активной поддержки. При изменениях ядра приоритет — на
> совместимость с `cli` и `tui`.
