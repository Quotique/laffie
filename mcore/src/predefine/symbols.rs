...
fn all_symbols() -> &'static RwLock<MultiMap<u64, String, Symbol>> {
    static INSTANCE: OnceCell<RwLock<MultiMap<u64, String, Symbol>>> = OnceCell::new();
...
pub fn add_with_name(symbols: &mut MultiMap<u64, String, Symbol>, name: &str) {
    add_symbol_impl(symbols, Symbol::builder().name(name).build().unwrap());
}

fn add_symbol_impl(symbols: &mut MultiMap<u64, String, Symbol>, mut symbol: Symbol) -> Symbol {
    if let Some(s) = symbols.get_mut_alt(&symbol.name) {
        s.attrs.extend(symbol.attrs.into_iter());
        return s.clone();
    }

    static LAST_ID: OnceCell<RwLock<u64>> = OnceCell::new();
    let last_id = LAST_ID.get_or_init(|| RwLock::new(0));
