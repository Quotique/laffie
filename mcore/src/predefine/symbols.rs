...
fn all_func_symbols() -> &'static RwLock<HashMap<CompactString, Arc<FuncSymbol>>> {
...
pub fn symbol_by_name(name: &str) -> Option<Arc<FuncSymbol>> {
    all_func_symbols()
        .read()
...
pub fn add_symbol(symbol: FuncSymbol) -> Arc<FuncSymbol> {
    add_symbol_impl(&mut all_func_symbols().write(), symbol)
}

pub fn add_with_name(symbols: &mut HashMap<CompactString, Arc<FuncSymbol>>, name: &str) {
    add_symbol_impl(symbols, FuncSymbol::builder().name(name).build());
}
