...
pub(super) fn all_func_symbols() -> &'static RwLock<HashMap<CompactString, SymbolProgram>> {
    ...
    ALL_SYMBOLS.get_or_init(|| {
        ...
        add_symbol_impl(
            ...
            SymbolProgram {
                name: "find".into(),
                attrs: HashMap::from([(SymbolAttr::Commutative, SymbolAttrValue::None)]),
                ..Default::default()
            },
        );
        SymbolProgram::add_with_name(&mut result, "prove");
        SymbolProgram::add_with_name(&mut result, "transform");
        SymbolProgram::add_with_name(&mut result, "solve");
        ...
        SymbolProgram::add_with_name(&mut result, "atom");
        SymbolProgram::add_with_name(&mut result, "even");

        add_symbol_impl(&mut result, ops::answer::symbol());
