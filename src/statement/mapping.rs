...
pub struct Mapper<'a> {
    target:  &'a StatementNode,
    pattern: &'a StatementNode,
}
...
impl ParamsMapping {
    pub fn mapper<'a>(target: &'a StatementNode, pattern: &'a StatementNode) -> Mapper<'a> {
    ...
    pub fn apply<'a>(&self, node: &'a mut StatementNode) -> &'a mut StatementNode {
...
#[cfg(test)]
mod tests {
    use parser::{statement_with_params, statement_with_vars};
    use predefine::setup;
    ...
    fn placeholder_false_test() {
        ...
        assert!(ParamsMapping::mapper(statement.root(), pattern.root())
            .try_map()
            .is_err());
    }
}
